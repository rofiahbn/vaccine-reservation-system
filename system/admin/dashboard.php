...
if ($now_serving) {
    ...
    // 1. Ambil semua participant (parent + child)
    $sql_participants = "
    ...
    // 2. Ambil layanan dengan penanganan Error (Try-Catch)
    // Inisialisasi array agar tidak error saat dipanggil di view
    $services_layanan_now = [];
    $services_paket_now = [];

    try {
        // Coba Query Baru (dengan kolom 'tipe')
        $sql_services = "
        ...
        while ($row = $result_s->fetch_assoc()) {
            if (isset($row['tipe']) && $row['tipe'] === 'paket') {
        ...
        // Jika kolom 'tipe' tidak ada, jalankan Query Fallback
        $sql_fallback = "
        ...
        $stmt_f = $conn->prepare($sql_fallback);
        $stmt_f->bind_param("ii", $parent_id, $parent_id);
        $stmt_f->execute();
...
<head>
    ...
    <link rel="stylesheet" href="system/admin/css/detail.css"> 
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
	 
    <script src="system/admin/js/admin.js"></script>
    ...
    <div class="main-content">
        ...
        <div class="booking-list">
            ...
            <table class="list-table">
                ...
                <?php
// 1. Cek keberadaan kolom 'tipe' SEKALI SAJA di luar loop
$has_tipe = false;
$res_check = $conn->query("DESCRIBE booking_services");
while ($row_check = $res_check->fetch_assoc()) {
    if ($row_check['Field'] === 'tipe') {
        $has_tipe = true;
        break;
    }
}
...
<tbody>
    <?php foreach ($grouped_bookings as $group): ?>
        ...
        // --- QUERY STAFF ---
        $sql_staff = "SELECT s.gelar, s.nama_lengkap FROM booking_staff bs JOIN staff s ON bs.staff_id = s.id WHERE bs.booking_id = ?";
        ...
        // --- QUERY SERVICES (OPTIMIZED) ---
        if ($has_tipe) {
            $sql_services = "SELECT bs.nama_layanan, bs.tipe 
                             FROM booking_services bs 
                             JOIN bookings b ON bs.booking_id = b.id 
                             WHERE b.id = ? OR b.parent_id = ? 
                             ORDER BY CASE bs.tipe WHEN 'pelayanan' THEN 1 WHEN 'paket' THEN 2 END, bs.nama_layanan ASC";
        } else {
            $sql_services = "SELECT bs.nama_layanan 
                             FROM booking_services bs 
                             JOIN bookings b ON bs.booking_id = b.id 
                             WHERE b.id = ? OR b.parent_id = ? 
                             ORDER BY bs.nama_layanan ASC";
        ...
        while ($s = $services->fetch_assoc()) {
            // Jika kolom tipe ada dan nilainya 'paket', masukkan ke paket
            if ($has_tipe && isset($s['tipe']) && $s['tipe'] === 'paket') {
                $services_paket[] = $s['nama_layanan'];
            } else {
                // Semua masuk layanan jika tidak ada kolom tipe ATAU tipenya 'pelayanan'
        ...
        <tr>
            <td>
                ...
                <?php foreach ($group as $p): ?>
                    <?php if ($p['id'] != $parent['id']): ?>
                        <div style="font-size:13px; margin-left:10px;">- <?= htmlspecialchars($p['nama_lengkap']) ?></div>
                    <?php endif; ?>
                <?php endforeach; ?>
            </td>
            ...
            <td>
                ...
                <?php if (empty($services_layanan) && empty($services_paket)): ?>-<?php endif; ?>
            </td>

            <td><?= substr($parent['waktu_booking'], 0, 5) ?> WIB</td>

            <td>
                <?php if (!empty($dokters)): ?>
                    <?php foreach ($dokters as $dokter): ?><div><?= $dokter ?></div><?php endforeach; ?>
                <?php else: ?>-<?php endif; ?>
            </td>
                ...
                <span class="status-badge <?= $parent['status'] ?>">
                    <?php 
                        $status_map = [
                            ...
                            'confirmed' => 'Pasien Dalam Antrian',
                            'completed' => 'Pesanan Selesai',
                            'cancelled' => 'Pesanan Dibatalkan'
                        ];
                        echo $status_map[$parent['status']] ?? 'Unknown';
                    ?>
                </span>
            </td>
        </tr>
