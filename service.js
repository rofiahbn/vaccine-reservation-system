        ...
        productData[category].forEach(product => {
            ...
            html += `
                <label class="product-checkbox">
                    ...
                    >
                    <span>${product.name} <small style="color:#666;">(Rp ${product.price.toLocaleString()})</small></span>
                </label>
            `;
        });
...
function toggleProduct(id, name) {
    ...
    if (index > -1) {
        ...
        // Ambil data service dari rawServices (hasil DB dari order.php)
        const product = rawServices.find(p => p.id == id);

        // Tambah ke array + simpan harga
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 🔥 JIKA EDIT MODE DAN ADA DATA LAMA → TAMPILKAN BADGE + ISI HIDDEN INPUT
    if (selectedProducts.length > 0) {
        updateBadges();
        updateHiddenInput();
    }
